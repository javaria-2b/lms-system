...
export async function student_login() {
  ...
  let stu = studentsData.find(
    (x) =>
      x.studentEmail === user._email && x.studentPassword === user._password
  ); // to access current student in dataBase
  ...
  async function user_dashboard() {
    ...
    switch (dashboard.operations) {
      case "Profile": {
        ...
        console.log(
          chalk.yellow(
            ` Courses Enrolled: ${stu?.coursesEnrolled
              .flatMap((x) => x.course_name)
              .join(", ")}`
          )
        );
        break;
    ...
    dashboard.operations !== "Log Out"
      ? await user_dashboard()
...
export async function instructors_login() {
  ...
  let ins = instructorsData.find(
    (x) =>
      x.instructorEmail === user._email &&
      x.instructorPassword === user._password
  ); // to access current instructors in dataBase
    ...
    let dashboard = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          "Assigned Courses",
          "Enrolled Students",
          "Apply To Teach Other Courses Offered",
          "Announcments",
          "Log Out",
        ],
      },
